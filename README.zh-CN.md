...
这是一个 Pi 扩展：第一次请求模型时，只提供一个刻意精简的工作环境；从后续请求开始，恢复 Pi 的正常提示词、上下文、工具和输出上限。

设计灵感来自
[`xiaobright/dsh-anchored-standard`](https://github.com/xiaobright/dsh-anchored-standard)。
本项目通过 Pi 的扩展 API 实现相同的“首轮请求塑形”思路。

## 它改变了什么

### 只精简第一次请求

空白会话第一次向模型 API 发起请求时，模型只能看到：
...
1. `session_start` 和 `before_agent_start` 检查已保存的对话。只要会话中已有 assistant 回复或工具结果，就视为已经提升。
2. 第一次调用模型之前，`before_agent_start` 保存 Pi 的正常系统提示词，并暂时只启用 `bash` 和 `read`。
3. `before_provider_request` 重写最终发送给模型 API 的第一次请求，确保提示词、消息、工具定义和 1,024-token 上限完全符合预期。
4. `tool_call` 和 `message_end` 检测第一次提升事件，并恢复 Pi 已注册的全部工具。
5. 如果第一次回复被 token 上限截断，`message_end` 会通过 Pi 的 `deliverAs: "steer"` 模式，在同一次运行中继续一次。

本扩展不单独保存阶段文件，也不写入额外数据库。已保存的对话就是状态来源，因此恢复、分叉或重新加载会话时，行为仍然正确。如果 `bash` 或 `read` 不可用，扩展会保留全部 Pi 工具，而不是给模型一个残缺的首轮环境。
...
只有在你明确希望第一次请求也保留 Pi 正常提示词和上下文时，才应把 `minimalPrompt` 设为 `null`。
...
这些动画来自仓库中实际 HTML 文件的浏览器录制。被测试的 agent 没有自行预览或验证输出。`max` 思考级别下，早期“新建后续轮次”方案生成的 SVG 布局大小为 `0×0`，所以录制结果是空白的；当前方案的两次输出都能正常渲染。
...
测试覆盖第一次请求塑形、会话状态判断、工具恢复、提示词恢复、首轮工具缺失、截断回复续写，以及轨迹统计。
